<?
	class PollerProcess implements IProcess
    {
        ...
        public function StartThread($farminfo)
        {
            ...
            try
            {
	            $records = $db->GetAll("SELECT * FROM records WHERE rtype='A' AND issystem='1' AND zoneid IN (SELECT id FROM zones WHERE farmid = '{$farminfo['id']}')");
	            $malformed_zones = array();
	            if ($records && count($records) > 0)
	            {
		            foreach ($records as $record)
		            {
		            	if (!$db->GetOne("SELECT id FROM farm_instances WHERE farmid='{$farminfo['id']}' AND (external_ip = '{$record['rvalue']}' OR internal_ip = '{$record['rvalue']}')"))
		            	{
                ...
                if ($db_ami["replace_to_ami"])
                {
                 	...
                 	if ($num_instances == 0)
                    {
                        ...
                        if ($sync_complete)
                        {
                        	...
                        	$db->Execute("UPDATE farm_amis SET ami_id='{$db_ami["replace_to_ami"]}', replace_to_ami='' WHERE ami_id='{$ami}'");
                        	...
                        	$db->Execute("UPDATE zones SET ami_id='{$db_ami["replace_to_ami"]}', role_name='{$role_name}' WHERE ami_id='{$ami}'");
                        	$db->Execute("UPDATE real_servers SET ami_id='{$db_ami["replace_to_ami"]}' WHERE ami_id='{$ami}'");
                        }
                    }
                    else 
                    {
				...
				*/
                ...
                if ($need_terminate_instance)
                {
                    if (count($role_instances_by_time) > $db_ami_info["min_count"])
                    {
                        ...
                        $instanceinfo = $db->GetRow("SELECT * FROM farm_instances WHERE instance_id=?", array($item->instanceId));
