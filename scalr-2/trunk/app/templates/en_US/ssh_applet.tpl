...
<body>
	<div style="margin-top:22px;margin-bottom:38px;font-weight:bold;font-size:10pt;font-family:Verdana;">{t}SSH console on server{/t}: <a title="View extended server info" href="/server_view_extended_info.php?server_id={$DBServer->serverId}">{$DBServer->serverId}</a> &nbsp; {t}IP{/t}: {$DBServer->remoteIp} &nbsp; {t}Internal IP{/t}: {$DBServer->localIp}</div>
