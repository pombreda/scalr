		...
		<tr valign="top">
			<td>
				{include file="inc/table_header.tpl" nofilter=1}
					...
					{include file="inc/intable_header.tpl" header="Clients (By account type)" color="Gray"}
						{section name=id loop=$packages}
						<tr>
							<td width="30%">{$packages[id].name} / ${$packages[id].cost}:</td>
							<td>{$packages[id].clients}</td>
						</tr>
						{/section}
					{include file="inc/intable_footer.tpl" color="Gray"}
					...
					{include file="inc/intable_header.tpl" header="Roles" color="Gray"}
						<tr>
							<td>Total active:</td>
							<td>{$roles.total} {if $roles.total > 0}[<a href="roles_view.php">View</a>]{/if}</td>
						</tr>
						<tr>
							<td>Shared:</td>
							<td>{$roles.shared} {if $roles.shared > 0}[<a href="roles_view.php?type=SHARED">View</a>]{/if}</td>
						</tr>
						<tr>
							<td>Custom:</td>
							<td>{$roles.custom} {if $roles.custom > 0}[<a href="roles_view.php?type=CUSTOM">View</a>]{/if}</td>
						</tr>
						<tr>
							<td>Contributed (Approved):</td>
							<td>{$roles.approved} {if $roles.approved > 0}[<a href="roles_view.php?type=SHARED&approval_state=Approved">View</a>]{/if}</td>
						</tr>
						<tr>
							<td>Contributed (Pending):</td>
							<td>{$roles.pending} {if $roles.pending > 0}[<a href="roles_view.php?type=SHARED&approval_state=Pending">View</a>]{/if}</td>
						</tr>
						<tr>
							<td>Contributed (Declined):</td>
							<td>{$roles.declined} {if $roles.declined > 0}[<a href="roles_view.php?type=SHARED&approval_state=Declined">View</a>]{/if}</td>
						</tr>
					{include file="inc/intable_footer.tpl" color="Gray"}
