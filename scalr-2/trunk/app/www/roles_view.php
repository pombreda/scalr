<?
	...
	if ($req_saved) {
		$okmsg = _("Role successfully saved");
	...
	if ($_POST && ($post_with_selected || $post_confirm))
	{
		if ($post_action == "delete" && count($post_id) != 0)
		{
			if (!$post_confirm)
			{
				...
				{
					$info = $db->GetRow("SELECT * FROM roles WHERE id=? AND env_id=?",
						array($roleid, Scalr_Session::getInstance()->getClientId())
					);
			...
			else
			{
				...
				foreach ((array)$post_id as $v)
				{
					try
					{
						...
						if ($DBRole->clientId == 0 || !Scalr_Session::getInstance()->getAuthToken()->hasAccessEnvironment($DBRole->envId))
							continue;
					}
					catch(Exception $e)
					{
