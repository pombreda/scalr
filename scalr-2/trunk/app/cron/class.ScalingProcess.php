	...
	class ScalingProcess implements IProcess
    {
        ...
        public function StartThread($farminfo)
        {
            ...
            {            	
            	...
            	{
            		$this->Logger->warn("[FarmID: {$DBFarm->ID}] Role '{$DBFarmRole->GetRoleName()}' being synchronized. This role will not be scalled.");
            		continue;
            	}
						...
						*/    
                        ...
                        if ($DBServer && $got_valid_instance)
                        {
                        	...
                        	if ($DBServer->platform == SERVER_PLATFORMS::EC2)
                        	{
					            ...
					            $AmazonEC2Client = AmazonEC2::GetInstance(AWSRegions::GetAPIURL($DBServer->GetProperty(EC2_SERVER_PROPERTIES::REGION))); 
            		...
            		elseif ($res == ScalingAlgo::UPSCALE)
	            	{
						...
						*/	            		
						...
						try {
							...
							Logger::getLogger(LOG_CATEGORY::FARM)->info(new FarmLogMessage($DBFarm->ID, sprintf("Farm %s, role %s scaling up (Algo: %s, Sensor value: %s). Starting new instance. ServerID = %s.", 
								$DBFarm->Name,
	                        	$DBServer->GetFarmRoleObject()->GetRoleName(),
	                        	get_class($ScalingAlgo),
	                        	is_array($ScalingAlgo->lastSensorValue) ? serialize($ScalingAlgo->lastSensorValue) : $ScalingAlgo->lastSensorValue,
