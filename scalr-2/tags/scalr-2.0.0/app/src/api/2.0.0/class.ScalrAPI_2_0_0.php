<?php
	...
	class ScalrAPI_2_0_0 extends ScalrAPICore
	{
		...
		public function DNSZonesList()
		{
			...
			$rows = $this->DB->Execute("SELECT * FROM dns_zones WHERE client_id=?", array($this->Client->ID));
		...
		{
			$zoneinfo = $this->DB->GetRow("SELECT id, client_id FROM dns_zones WHERE zone_name=?", array($ZoneName));
		...
		{
			$zoneinfo = $this->DB->GetRow("SELECT id, client_id, allow_manage_system_records FROM dns_zones WHERE zone_name=?", array($ZoneName));
			if (!$zoneinfo || $zoneinfo['client_id'] != $this->Client->ID)
				throw new Exception (sprintf("Zone '%s' not found in database", $ZoneName));
		...
		public function DNSZoneCreate($DomainName, $FarmID = null, $FarmRoleID = null)
		{
			...
			while (count($domain_chunks) > 0)
			{
				...
				if ($this->DB->GetOne("SELECT id FROM dns_zones WHERE zone_name=? AND client_id != ?", array($chk_dmn, $_SESSION['uid'])))
				{
		...
		public function FarmGetStats($FarmID, $Date = null)
		{
			...
			$farminfo = $this->DB->GetRow("SELECT id FROM farms WHERE id=? AND clientid=?",
		...
		public function FarmsList()
		{
			...
			$farms = $this->DB->Execute("SELECT id, name, region, status, comments FROM farms WHERE clientid = ?", array($this->Client->ID));
		...
		{
			$farminfo = $this->DB->GetRow("SELECT clientid FROM farms WHERE id=? AND clientid=?",
				array($FarmID, $this->Client->ID)
			);
		...
		public function RolesList($Platform = null, $ImageID = null, $Name = null, $Prefix = null)
		{
			...
			$sql = "SELECT * FROM roles WHERE (clientid='0' OR clientid='{$this->Client->ID}')";
			...
			if ($Platform)
				$sql .= " AND platform = {$this->DB->qstr($Name)}";
		...
		public function BundleTaskGetStatus($BundleTaskID)
		{
			...
			if ($BundleTask->clientId != $this->Client->ID)
		...
		public function ServerImageCreate($ServerID, $RoleName)
		{
    		...
    		if ($DBServer->clientId != $this->Client->ID)
            ...
            if ($chk && $chk != $DBServer->serverId)
            {
            	try
            	{
	            	...
	            	if ($bDBServer->farmId == $DBServer->farmId)
	            		throw new Exception(sprintf(_("Role '%s' is already synchonizing."), $DBServer->GetFarmRoleObject()->GetRoleName()));
            	}
            	catch(Exception $e) {}
            }
            
            
            $role = $this->DB->GetOne("SELECT id FROM roles WHERE name=? AND clientid=?", array($RoleName, $DBServer->clientId));
            if (!$role)
            {
