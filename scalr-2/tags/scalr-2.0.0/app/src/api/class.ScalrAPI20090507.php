<?php
	...
	class ScalrAPI20090507 extends ScalrAPICore
	{
		...
		{			
			...
			$sql = "SELECT * FROM roles WHERE platform='".SERVER_PLATFORMS::EC2."' AND (clientid='0' OR clientid='{$this->Client->ID}') AND region={$this->DB->qstr($Region)}";
		...
		public function ListApplications()
		{
			...
			while ($row = $rows->FetchRow())
			{
				...
				{
					$instances = $this->DB->GetAll("SELECT rvalue FROM dns_zone_records WHERE zone_id=? AND `type`=? AND `name`=? AND `issystem`=?",
						array($row['id'], 'A', '@', 1)
					);
					...
					{
						$itm_ip = new stdClass();
						$itm_ip->IPAddress = $instance['value'];
