...
{	if (_isIE)try {document.execCommand("BackgroundImageCache", false, true);}catch(e){};if (typeof(htmlObject)!="object")
 ...
 this.onDragEnd = function(id) {};
 
 this.htmlNode=new dhtmlXTreeItemObject(this.rootId,"",0,this);this.htmlNode.htmlNode.childNodes[0].childNodes[0].style.display="none";
 ...
 if (_isIE)this.preventIECashing(true);var self=this;if (window.addEventListener)window.addEventListener("unload",function(){try{self.destructor();}catch(e){}},false);if (window.attachEvent)window.attachEvent("onunload",function(){try{self.destructor();}catch(e){}});this.dhx_Event();return this;};dhtmlXTreeObject.prototype.enableIEImageFix=function(mode){if (!mode){this._getImg=function(id){return document.createElement((id==this.rootId)?"div":"img");};
 
 this.OnXMLLoaded = null;
        ...
        { 
            switch(tempStr[i])
            {
                ...
                case "HIDECHECK":
                    try
                    {                
                        Nodes[Count].htmlNode.childNodes[0].childNodes[0].childNodes[1].style.display = "none";
                    }
                    catch(err){}
 ...
 { 	
 	...
 	for(var i=start||0;i<node.childNodes.length;i++)
 	{
 		if ((node.childNodes[i].nodeType==1)&&(node.childNodes[i].tagName == "item"))
 		{
 			...
 			var hdch=nodx.getAttribute("hidecheck"); 
 			...
 			var unstable = nodx.getAttribute("unstable");
 			var zST="";
 			
 			if (aselect)zST+=",SELECT";
 			if (atop)zST+=",TOP";
 			if (hdch)zST+=",HIDECHECK";
 			...
 			if (unstable=="1")
 			{
 				newNode.htmlNode.unstableIcon.style.display = '';
 			}
 	...
 	if (!level)
 	{
 		...
 		dhtmlObject.lastLoadedXMLId=parentId;dhtmlObject.XMLloadingWarning=0;
 ...
 dhtmlXTreeObject.prototype._branchUpdateNext=function(node){var items=this.XMLLoader.doXPath("./item",node);for (var i=0;i<items.length;i++){this._branchUpdate++;this.smartRefreshItem(items[i].getAttribute("id"),items[i]);};this._branchUpdate--;};
 ...
 dhtmlXTreeObject.prototype._correctPlus=function(itemObject)
 {
 	...
 	if ((this.XMLsource)&&(!itemObject.XMLload))
 	{
 		...
 		{  
 			//Scalr improvement
 			if (itemObject.images[0] != 'leaf.gif')
 	...
 	dhtmlXTreeObject.prototype._correctLine=function(itemObject){var sNode=itemObject.parentObject;if (sNode)if ((this._getLineStatus(itemObject.id,sNode)==0)||(!this.treeLinesOn))
 ...
 dhtmlXTreeObject.prototype._selectItem=function(node,e)
 {
 	...
 	if (this._onSSCF)
 	{
 		...
 		{
 			allowSelect = this.callEvent("onSelect",[z]);
 		}
 	};
 ...
 dhtmlXTreeObject.prototype._createItem=function(acheck,itemObject,mode)
 {
	...
	var td2=document.createElement('td');
	td2.className="standartTreeRow TreeContentContainer";
	...
	*/
	...
	img.src = "/images/farm_tree/unstable.png";
	img.style.float = 'right'
	img.style.display = 'none';
	img.title = "Unstable - has not been tested enough yet";
	...
	if (this.mlitems)
	{
		...
		itemObject.span.style.overflow="hidden";
	}
	else 
		td2.noWrap=true;
	...
	itemObject.span.style.paddingLeft="1px";
 ...
 {var temp=this._globalIdStorageFind(itemId);if (!temp)return 0;this._xopenAll(temp);};dhtmlXTreeObject.prototype.getOpenState=function(itemId){var temp=this._globalIdStorageFind(itemId);if (!temp)return "";return this._getOpenState(temp);};
 ...
 dhtmlXTreeObject.prototype.enableDragAndDrop=function(mode,rmode){if (mode=="temporary_disabled"){this.dADTempOff=false;mode=true;}else
 ...
 dhtmlXTreeObject.prototype._setMove=function(htmlNode,x,y)
 {
	if(htmlNode.parentObject.span)
    {
        ...
        //_debug("_setMove(htmlNode,x,y){ line 2915: this.dadmode = "+this.dadmode+"; }");
        ...
        if(this.dadmode==2)
        {
            ...
            if (/^list_/.test(htmlNode.parentObject.id))
                this.dadmodec=0;
            else
            {
                this.dadmodec=1;
 ...
 this.allTree.scrollTop=z;};dhtmlXTreeObject.prototype._preventNsDrag=function(e){if ((e)&&(e.preventDefault)) {e.preventDefault();return false;};return false;};
