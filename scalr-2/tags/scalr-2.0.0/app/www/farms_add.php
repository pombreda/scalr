<? 
    ...
    if ($req_id)
    {
        ...
        $roles = $db->GetAll("SELECT * FROM farm_roles WHERE farmid=? ORDER BY launch_index ASC", array($DBFarm->ID));
        $display['roles'] = array();
        foreach ($roles as &$row)
        {
            $role_info = $db->GetRow("SELECT * FROM roles WHERE id=?", array($row['role_id']));
            if (!$role_info)
            	continue;
            
        	$row["role"] = $role_info["name"];
        	
        	$scripts = $db->GetAll("SELECT * FROM farm_role_scripts WHERE farm_roleid=? AND event_name NOT LIKE 'CustomEvent%'", array($row['id']));
	        ...
	        if ($role_info['roletype'] == ROLE_TYPE::SHARED && $role_info['clientid'] != 0)
	        {
	        	$author_info = $db->GetRow("SELECT fullname FROM clients WHERE id=?", array($role_info['clientid']));
        	...
        	{
		        if (!$role_settings['mysql.pbw1_hh'])
		        	$role_settings['mysql.pbw1_hh'] = '05';
		        	
		        if (!$role_settings['mysql.pbw2_hh'])
		        	$role_settings['mysql.pbw2_hh'] = '09';
		        
		        if (!$role_settings['mysql.pbw1_mm'])
		        	$role_settings['mysql.pbw1_mm'] = '00';
		        	
		        if (!$role_settings['mysql.pbw2_mm'])
		        	$role_settings['mysql.pbw2_mm'] = '00';

		        if (!$role_settings[DBFarmRole::SETTING_MYSQL_BUNDLE_EVERY])
					$role_settings[DBFarmRole::SETTING_MYSQL_BUNDLE_EVERY] = 48;
					
				if (!$role_settings[DBFarmRole::SETTING_MYSQL_BCP_EVERY])
					$role_settings[DBFarmRole::SETTING_MYSQL_BCP_EVERY] = 180;
					
				if (!$role_settings[DBFarmRole::SETTING_MYSQL_DATA_STORAGE_ENGINE])
					$role_settings[DBFarmRole::SETTING_MYSQL_DATA_STORAGE_ENGINE] = MYSQL_STORAGE_ENGINE::EBS;
		        	
		       	if (!$role_settings[DBFarmRole::SETTING_MYSQL_EBS_VOLUME_SIZE])
						$role_settings[DBFarmRole::SETTING_MYSQL_EBS_VOLUME_SIZE] = 100;
        	}
	        	
	        $role = array(
	        	'role_id'	=> $role_info['id'],
        		'name' 		=> $role_info["name"],
        		'arch' 		=> $role_info["architecture"],
        		'alias' 	=> $role_info["alias"],
        		'ami_id'	=> $role_info["ami_id"],
	        	'platform'	=> $role_info['platform'],
        		'type'		=> ROLE_ALIAS::GetTypeByAlias($role_info["alias"]),
        	...
        	if ($DBFarmRole->GetRoleAlias() == ROLE_ALIAS::MYSQL)
        		$display['farm_mysql_role'] = $role_info['id'];
