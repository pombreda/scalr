<?php
	...
	{			
		...
		{
			if (!$this->db->GetOne("SELECT id FROM role_images WHERE image_id = ? AND role_id != ?", array($imageId, $this->id)))
			{
		...
		public static function createFromBundleTask(BundleTask $BundleTask)
		{
			...
			{
				$DBServer = DBServer::LoadByID($BundleTask->serverId);
				if (in_array($DBServer->platform, array(SERVER_PLATFORMS::EC2, SERVER_PLATFORMS::EUCALYPTUS)))
				{
					$proto_role = array(
