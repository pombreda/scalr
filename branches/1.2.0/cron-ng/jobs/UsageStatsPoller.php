<?php
	...
	class Scalr_Cronjob_UsageStatsPoller extends Scalr_System_Cronjob_MultiProcess_DefaultWorker {
    	
    	static function getConfig () {
    		return array(
    			"description" => "Farm usage stats poller"
    		);
    	}
        ...
        function handleWork ($farmId)
        {
            $farminfo = $this->db->GetRow("SELECT farms.*, clients.isactive FROM farms 
            	INNER JOIN clients ON clients.id = farms.clientid WHERE clients.isactive='1' AND farms.id=?",
            	array($farmId)
            );
            ...
            $this->logger->info("[{$GLOBALS["SUB_TRANSACTIONID"]}] Begin polling usage stats for farm (ID: {$farminfo['id']}, Name: {$farminfo['name']})");
            ...
            $farm_roles = $this->db->GetAll("SELECT * FROM farm_roles WHERE farmid='{$farminfo['id']}'");
            $this->logger->debug("[FarmID: {$farminfo['id']}] Farm used ".count($farm_roles)." AMIs");
            
            $farm_instances = $this->db->GetAll("SELECT * FROM farm_instances WHERE farmid='{$farminfo['id']}'");
            ...
            if (is_array($result->reservationSet->item))
            {
                ...
                {
					$ami_role_name = $this->db->GetOne("SELECT role_name FROM farm_instances WHERE instance_id=? AND farmid=?", 
            ...
            $db_amis = $this->db->Execute("SELECT * FROM farm_roles WHERE farmid=?", array($farminfo["id"]));          
            while ($db_ami = $db_amis->FetchRow())
            {
                ...
                $role = $this->db->GetOne("SELECT name FROM roles WHERE ami_id=?", array($db_ami['ami_id']));
                ...
                {                	
                	$db_item_info = $this->db->GetRow("SELECT * FROM farm_instances WHERE instance_id=? AND farmid=?", array($item->instanceId, $farminfo["id"]));                        
                    if ($db_item_info)
                    {
                        ...
                        if ($item->instanceState->name == 'running' && ($db_item_info["state"] == INSTANCE_STATE::RUNNING))
                        {
                            if ($db_item_info["isrebootlaunched"] == 0)
                            {
                                ...
                                {                                	
						            ...
						            $this->db->Execute("UPDATE farm_instances SET bwusage_in=?, bwusage_out=?, dtlaststatusupdate=UNIX_TIMESTAMP(NOW()) WHERE id=?",
			...
			$current_stat = $this->db->GetRow("SELECT * FROM farm_stats WHERE farmid=? AND month=? AND year=?",
			...
			{				
				foreach ($items as $item)
				{
					...
					$last_uptime = $this->db->GetOne("SELECT uptime FROM farm_instances WHERE instance_id=?", array($item->instanceId));
					$uptime_delta = $uptime-$last_uptime;
	                    
					$stat_uptime[$item->instanceType] += $uptime_delta;
					
					$this->db->Execute("UPDATE farm_instances SET uptime=? WHERE instance_id=?",
			...
			if (!$current_stat)
			{
				$this->db->Execute("INSERT INTO farm_stats SET farmid=?, month=?, year=?",
					array($farminfo['id'], date("m"), date("Y"))
				);
			}
			...
			$this->db->Execute("UPDATE farm_stats SET 
                bw_in		= bw_in+?, 
                bw_out		= bw_out+?, 
                m1_small	= m1_small+?,
                m1_large	= m1_large+?,
