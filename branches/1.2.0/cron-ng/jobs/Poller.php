<?php
	...
	class Scalr_Cronjob_Poller extends Scalr_System_Cronjob_MultiProcess_DefaultWorker {
        ...
        function handleWork ($farmId) {
            ...
            $farm_instances = $this->db->GetAll("SELECT * FROM farm_instances WHERE farmid=? AND state != ?",
            ...
            if (is_array($result->reservationSet->item))
            {
                ...
                {
					$ami_role_name = $this->db->GetOne("SELECT role_name FROM farm_instances WHERE instance_id=? AND farmid=? AND state != ?", 
            ...
            // Check farm status
            //
            if ($this->db->GetOne("SELECT status FROM farms WHERE id=?", array($DBFarm->ID)) != FARM_STATUS::RUNNING)
            {
            	$this->logger->debug("[FarmID: {$DBFarm->ID}] Farm is not running.");
            	return;
            }
            
            $db_amis = $this->db->Execute("SELECT id FROM farm_roles WHERE farmid=? ORDER BY launch_index ASC", array($DBFarm->ID));
            ...
            while ($db_ami = $db_amis->FetchRow())
            {
                ...
                if ($DBFarmRole->ReplaceToAMI)
                {
                	...
                	$num_instances = $this->db->GetOne("SELECT COUNT(*) FROM farm_instances WHERE ami_id=? AND farmid = ? AND state != ?", array(
                		$DBFarmRole->AMIID,
                		$DBFarmRole->FarmID,
                 	...
                 	if ($num_instances == 0)
                    {
                        ...
                        {
                        	if ($DBFarmRole->GetRoleName() == $this->db->GetOne("SELECT name FROM roles WHERE ami_id='{$DBFarmRole->ReplaceToAMI}'"))
                            {
                            	if ($this->db->GetOne("SELECT COUNT(*) FROM farm_instances WHERE ami_id='{$DBFarmRole->AMIID}' AND state != '".INSTANCE_STATE::TERMINATED."' AND farmid IN (SELECT id FROM farms WHERE clientid='{$DBFarm->ClientID}')") == 0)
                            	{
                        ...
                        if ($sync_complete)
                        {
                        	$new_role_name = $this->db->GetOne("SELECT name FROM roles WHERE ami_id='{$DBFarmRole->ReplaceToAMI}'");
                        	...
                        	{
                        		$this->db->Execute("UPDATE farm_roles SET ami_id='{$DBFarmRole->ReplaceToAMI}', replace_to_ami='' WHERE ami_id='{$DBFarmRole->AMIID}' AND farmid IN (SELECT id FROM farms WHERE clientid='{$DBFarm->ClientID}')");
                        		$this->db->Execute("UPDATE zones SET ami_id='{$DBFarmRole->ReplaceToAMI}', role_name='{$new_role_name}', isobsoleted='1' WHERE ami_id='{$DBFarmRole->AMIID}' AND clientid='{$DBFarm->ClientID}'");
	                        	if ($old_role_name != $new_role_name)
	                        	{
	                        		try
	                        		{
	                        			$this->db->Execute("UPDATE records SET `rkey` = REPLACE(`rkey`, '{$old_role_name}', '{$new_role_name}') WHERE issystem='1' AND zoneid IN (SELECT id FROM zones WHERE farmid IN (SELECT id FROM farms WHERE clientid='{$DBFarm->ClientID}'))");
	                        		}
	                        		catch(Exception $e){}
	                        	}                        		
                        	}
                        	...
                        	{
                        		$this->db->Execute("UPDATE farm_roles SET ami_id='{$DBFarmRole->ReplaceToAMI}', replace_to_ami='' WHERE ami_id='{$DBFarmRole->AMIID}' AND farmid='{$DBFarm->ID}'");
                        		$this->db->Execute("UPDATE zones SET ami_id='{$DBFarmRole->ReplaceToAMI}', role_name='{$new_role_name}', isobsoleted='1' WHERE ami_id='{$DBFarmRole->AMIID}' AND clientid='{$DBFarm->ClientID}' AND farmid='{$DBFarm->ID}'");
                        		if ($old_role_name != $new_role_name)
	                        	{
	                        		try
	                        		{
	                        			$this->db->Execute("UPDATE records SET `rkey` = REPLACE(`rkey`, '{$old_role_name}', '{$new_role_name}') WHERE issystem='1' AND zoneid IN (SELECT id FROM zones WHERE farmid='{$DBFarm->ID}')");
	                        		}
	                        		catch(Exeption $e){}
	                        	} 
                        	}
                        	...
                        	$this->db->Execute("UPDATE roles SET `replace`='' WHERE ami_id='{$DBFarmRole->ReplaceToAMI}'");
                        	
                        	if ($delete_old_ami)
                        	{
                        		$this->db->Execute("DELETE FROM roles WHERE ami_id=? AND roletype=?", array(
                        			$delete_old_ami,
                        			ROLE_TYPE::CUSTOM
                        		));
                        	}
                    ...
                    else 
                    {
                        ...
                        $chk = $this->db->GetOne("SELECT COUNT(*) FROM farm_instances WHERE state IN(?,?) AND ami_id='{$DBFarmRole->ReplaceToAMI}' AND farmid='{$DBFarm->ID}'", array(INSTANCE_STATE::PENDING, INSTANCE_STATE::INIT));
                        if ($chk != 0)
                        {
                   		...
                   		$chk = $this->db->GetOne("SELECT COUNT(*) FROM farm_instances WHERE state = ? AND ami_id='{$DBFarmRole->AMIID}' AND farmid='{$DBFarm->ID}'", array(INSTANCE_STATE::PENDING_TERMINATE));
                        if ($chk != 0)
                        {
                        ...
                        try 
            			{            
           					$old_instances = $this->db->GetAll("SELECT * FROM farm_instances WHERE ami_id='{$DBFarmRole->AMIID}' AND farmid='{$DBFarm->ID}' AND state NOT IN (?,?) ORDER BY id ASC", array(INSTANCE_STATE::PENDING_TERMINATE, INSTANCE_STATE::TERMINATED));
           					
           					$ami_info = $this->db->GetRow("SELECT * FROM roles WHERE ami_id='{$DBFarmRole->ReplaceToAMI}'");
            				if (!$ami_info)
           					{
           						$this->logger->warn("Cannot replace farm ami to new one. Role with ami '{$DBFarmRole->ReplaceToAMI}' was removed.");
           						$this->db->Execute("UPDATE farm_roles SET replace_to_ami='' WHERE id='{$DBFarmRole->ID}'");
           						continue;
           					}
            			    ...
            			    {            			    	
            			    	...
            			    	{
            			    		$this->db->Execute("UPDATE farm_instances SET ami_id=?, role_name=? WHERE id=?",
                                ...
                                if ($res)
                                {
                                    ...
                                    $this->db->Execute("UPDATE farm_instances SET replace_iid='{$old_instance['instance_id']}', `index`='{$old_instance['index']}' WHERE instance_id='{$res}'");
                                    break;
                                }
            			    }
            			}
                ...
                {                	
                	$db_item_info = $this->db->GetRow("SELECT * FROM farm_instances WHERE instance_id=? AND farmid=?", array($item->instanceId, $DBFarm->ID));                        
                    if ($db_item_info)
                    {
                        ...
                        if ($item->instanceState->name == 'running' && ($db_item_info["state"] == INSTANCE_STATE::RUNNING))
                        {
                            if ($db_item_info["isrebootlaunched"] == 0)
                            {
                                ...
                                if ($res === false)
                                {
                                	...
                                	if ($ip && $ip != $instance_dns && $ip != $db_item_info['external_ip'])
                                	{
                                		...
                                		$this->db->Execute("UPDATE elastic_ips SET ipaddress=? WHERE ipaddress=?",
                                			array($ip, $old_ip)
                                		);
                            ...
                            else 
                            {
                                ...
                                $reboot_timeout = (int)$this->db->GetOne("SELECT `reboot_timeout` FROM farm_roles WHERE farmid=? AND ami_id=? OR replace_to_ami=?", 
                        ...
                        elseif ($item->instanceState->name == 'running' && $db_item_info["state"] != INSTANCE_STATE::RUNNING)
                        {
                            ...
                            $launch_timeout = (int)$this->db->GetOne("SELECT `launch_timeout` FROM farm_roles WHERE farmid=? AND ami_id=? OR replace_to_ami=?", 
                            	array($DBFarm->ID, $db_item_info['ami_id'], $db_item_info['ami_id'])
                            );	
        ...
        function endForking () {
			...
			try
			{
	            ...
	            $roles = $this->db->GetAll("SELECT * FROM roles WHERE iscompleted='0' AND rebundle_trap_received='0'");
	            foreach ($roles as $ami_role)
	            {
	            	if (strtotime($ami_role['dtbuildstarted'])+$trap_wait_timeout < time())
	            	{
	            		...
	            		$this->db->Execute("UPDATE roles SET iscompleted='2', fail_details=?, `replace`='' WHERE id=?",
	            ...
	            $roles = $this->db->GetAll("SELECT * FROM roles WHERE iscompleted='0'");
	            foreach ($roles as $ami_role)
	            {
	            	$sync_timeout = $this->db->GetOne("SELECT `value` FROM client_settings WHERE `key`=? AND clientid=?", array('sync_timeout', $ami_role['client_id']));
	            ...
	            $this->db->Execute("DELETE FROM farm_instances WHERE state=? AND UNIX_TIMESTAMP(dtadded)+3600 < UNIX_TIMESTAMP(NOW())", array(INSTANCE_STATE::TERMINATED));
	            $this->db->Execute("DELETE FROM messages WHERE instance_id NOT IN (SELECT instance_id FROM farm_instances)");
			}
			catch (Exception $e)
			{
