{include file="inc/header.tpl"}
	...
	{   
		 ...
		 window.df = new DataForm($('role_data_form'));
...
{/literal}
	...
	{include file="inc/table_header.tpl" nofilter=1 tabs=1}
    	...
    	<tr>
    		<td width="20%">{t}Default mimimum LA for this role{/t}:</td>
    		<td>
    			<input type="text" class="text" name="default_minLA" id="default_minLA" value="{$default_minLA}" style="vertical-align:middle;" />
    		</td>
    	</tr>
    	<tr>
    		<td width="20%">{t}Default maximum LA for this role{/t}:</td>
    		<td>
    			<input type="text" class="text" name="default_maxLA" id="default_maxLA" value="{$default_maxLA}" />
    		</td>
    	</tr>
    	<tr valign="top">
    	...
    	<tr valign="top">
    		<td colspan="2">
    			...
    			{t}I did not  leave any sensible data (like passwords or private keys) on this instance.{/t}
    		</td>
    	</tr>
    	{if !$is_role_public}
    	<tr valign="top">
    	...
    	<script type="text/javascript">
			...
			{
				r = !$('no_keys_on_ami').checked;
				if ($('make_role_public'))
				{
					r = r | !$('make_role_public').checked;
				}
				
				$('button_js').disabled = r;
			}

			function CheckName()
			{
				$('name_check').style.display = '';
				...
				new Ajax.Request('/server/server.php?_cmd=check_role_name&name='+$('role_name').value+"&ami_id={/literal}{$ami_id}{literal}",
				{ 
					method: 'get',
					...
					{ 
						try
						{
							...
							{
								document.location.href = '/client_roles_view.php';
							}
							else
							{
								var err_obj = $('Webta_ErrMsg');
								err_obj.innerHTML = transport.responseText;
