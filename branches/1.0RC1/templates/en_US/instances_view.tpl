{include file="inc/header.tpl"}
	...
	<tr id='tr_{$smarty.section.id.iteration}'>
		...
		<td class="Item" valign="top">{$rows[id]->instancesSet->item->placement->availabilityZone}</td>
		...
		<td class="Item" valign="top">{if $rows[id]->instancesSet->item->instanceState->name == 'running' && $rows[id]->LA != 'Unknown'}<a href="syncronize_role.php?iid={$rows[id]->instancesSet->item->instanceId}">Synchronize to all</a>{/if}</td>
	...
	{sectionelse}
	<tr>
		<td colspan="12" align="center">No instances found</td>
	</tr>
	{/section}
	<tr>
