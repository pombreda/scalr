{include file="inc/header.tpl"}
	{include file="inc/table_header.tpl"}
    	...
    	<tr>
    		...
    		<td>
    			<select name="AvailabilityZone" class="text">
    				{section name=id loop=$avail_zones}
    				<option {if $POST.AvailabilityZone == $avail_zones[id]}selected{/if} value="{$avail_zones[id]}">{if $avail_zones[id] == ""}Choose randomly{else}{$avail_zones[id]}{/if}</option>
    				{/section}
    			</select>
    		</td>
    	</tr>
    	...
    	<tr>
    		...
    		<td>
    			<input type="text" class="text" size="2" name="BackupRetentionPeriod" value="{$POST.BackupRetentionPeriod}" />
    		</td>
    	</tr>
    	<tr>
