		...
		foreach ($db->GetAll($sql) as $row)
		{
			...
			switch($row['target_type'])
			{
				...
				case SCRIPTING_TARGET::INSTANCE: 	
						$instanceArgs = explode(":", $row['target_id']);
						
						try
						{
							$DBInstance = DBInstance::LoadByFarmRoleIDAndIndex($instanceArgs[0], $instanceArgs[1]);
							
							$row['target_name'] = "({$DBInstance->ExternalIP}) {$DBInstance->InstanceID}";
