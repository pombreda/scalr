<?
	...
	class SRSPlus extends Core
	{
		...
		function __construct($args)
		{
			...
			$this->GPG = new GnuPG(CF_GPG_PATH, CF_GPG_HOME);
						
			/*
			if ($this->TestMode)
				$this->Host = "testsrs.srsplus.com";
			else
				$this->Host = "srs.srsplus.com";
			*/
			$this->Host = $args["host"];
		}
		...
		*/
		...
		{			
			...
			{
				$this->RaiseWarning("Cannot create nameserver host ({$host})!");
				return false;
			}
		}
		...
		public function RenewDomain($domain, $TLD, $period)
		{
			...
			{
				$this->RaiseWarning("Cannot create domain name ({$domain}.{$TLD})!");
				return false;
			}
		 ...
		 * @return array
		 */
		public function CreateDomain($domain, $TLD, $period, $contacts, $ns = array())
		{
			$info = $this->DomainInfo($domain, $TLD);
			if (!$info)
			{
				$this->RaiseWarning("Cannot get domain information ({$domain}.{$TLD}) for register.");
				return false;
			}
			...
			{
				$this->RaiseWarning("Domain name ({$domain}.{$TLD}) is not avaiable.");
				return false;
			}
		...
		*/
		...
		private function ParseResponse($message)
		{
			...
			if ($this->VerifySRS($message))
			{
				...
				if ($headers["STATUS"] == "" || $headers["PROTOCOL VERSION"] == "")
					$this->RaiseWarning("Did not get properly formatted request from server");
				else
				{
			...
			{	
				// sign not property
				$this->RaiseWarning("Response verification failed!");
				return false;
			}
		}
		...
		function Log($str)
		{
			if (CF_DEV_DEBUG)
			{
				$hh = @fopen(LOGS_PATH."/transactions_".date("Ymd").".log", "a+");
				@fwrite($hh, date("d-m-Y H:i:s").": ".$str."\n--------------------------------------\n");
				@fclose($hh);
			}
		}
	}
