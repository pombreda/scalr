<?php
	...
	class PayPalDriver extends AbstractPaymentProcessorDriver implements IPaymentProcessorDriver
	{
		 ...
		 * @return bool
		 */
		public final function ProccessPayment($expected_amount = false, $expected_currency = false, $check_txn_type = true)
		{
			//
			$res = $this->PostBack();
			...
			(($_POST["payment_status"] == "Completed" || ($_POST["payment_status"] == "Pending" && CF_PAYMENTS_PAYPAL_DEBUG == 1)));
			...
			else
			{
				try 
    			{
        			...
        			$reason .= (strcmp($res, "VERIFIED")) ? "passed" : "failed ({$res})";
