<?
	class LAScalingAlgo extends ScalingAlgo implements IScalingAlgo
	{
		...
		public static function ValidateConfiguration(array &$config, DBFarmRole $DBFarmRole)
		{
			...
			if ($config["scaling.{$algo_name}.enabled"] == 1)
			{
				...
				if($config[self::PROPERTY_MAX_LA] <= 0 || $config[self::PROPERTY_MAX_LA] > 50)
					throw new Exception(sprintf(_("Maximum LA for role '%s' must be a number between 0.1 and 50"), $DBFarmRole->GetRoleName()));
		...
		public function MakeDecision(DBFarmRole $DBFarmRole)
		{
			...
			$sensor_value = $LASensor->GetValue($DBFarmRole);
			$this->Logger->info("LAScalingAlgo({$DBFarmRole->FarmID}, {$DBFarmRole->AMIID}) Sensor returned value: {$sensor_value}.");
