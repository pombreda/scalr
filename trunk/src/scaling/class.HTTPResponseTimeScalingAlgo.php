	...
	class HTTPResponseTimeScalingAlgo extends ScalingAlgo implements IScalingAlgo
	{
		...
		public static function ValidateConfiguration(array &$config, DBFarmRole $DBFarmRole)
		{
			...
			if ($config["scaling.{$algo_name}.enabled"] == 1)
			{
				...
				if($config[self::PROPERTY_MIN_RTIME] < 1)
					throw new Exception(sprintf(_("Minimum response time (HTTP response time scaling) for role '%s' must be a number greather than 0"), $DBFarmRole->GetRoleName()));
					
				$config[self::PROPERTY_MAX_RTIME] = (int)$config[self::PROPERTY_MAX_RTIME];
				if($config[self::PROPERTY_MAX_RTIME] < 2)
					throw new Exception(sprintf(_("Maximum response time (HTTP response time scaling) for role '%s' must be a number greather than 2"), $DBFarmRole->GetRoleName()));
					
				if($config[self::PROPERTY_MAX_RTIME] < $config[self::PROPERTY_MIN_RTIME])
					throw new Exception(sprintf(_("Maximum response time (HTTP response time scaling) for role '%s' must be greather than minimum response time"), $DBFarmRole->GetRoleName()));
					
				if (!$config[self::PROPERTY_URL] || $config[self::PROPERTY_URL] == '0')
					throw new Exception(sprintf(_("URL for role '%s' required"), $DBFarmRole->GetRoleName()));
			}
		...
		{			
			...
			try
			{
				...
				$sensor_value = $Sensor->GetValue($DBFarmRole);
				$this->Logger->info("HTTPResponseTimeScalingSensor({$DBFarmRole->FarmID}, {$DBFarmRole->AMIID}) Sensor returned value: {$sensor_value}.");
			}
			catch(Exception $e)
			{
			...
			if ($sensor_value > $this->GetProperty(self::PROPERTY_MAX_RTIME))
			{
			...
			elseif ($sensor_value < $this->GetProperty(self::PROPERTY_MIN_RTIME))
			{
